 ...
 *
 * Dojo Implementation
 * Copyright (c) 2010 Wade Taylor
 */

var chromaHash = function(inputNode, options) {
    var trigger = function(input, hashes) { 
        if(input.value === "" ) {
            hashes.forEach(function(item) {
                dojo.animateProperty({
                    node: item, 
                    properties: {backgroundColor: "#ffffff", opacity: 0}
                }).play();
            });
            return;
        }

        var position   = dojo.position(input, true);

        hashes.forEach(function(item, index) {
            var properties = {
                position:   'absolute',
                opacity:    1.0,
                left:       position.x + position.w - 1 + "px",
                top:        position.y + "px",
            ...
            dojo.style(item, properties);
        });

       
        var md5    = hex_md5('' + input.value + ':' + options.salt);
        var colors = md5.match(/([\dABCDEF]{6})/ig);
          
        if(input.value.length < options.minimum) {
            hashes.forEach(function(item, index) {
                var g = (parseInt(colors[index], 16) % 0xF).toString(16);
                dojo.animateProperty({
                    node: item, 
                    properties: {backgroundColor:"#" + g + g + g}
                }).play();
            });
        }
        else {
            hashes.forEach(function(item, index) {
                var color = parseInt(colors[index], 16);
                var red   = (color >> 16) & 255;
                var green = (color >> 8) & 255;
                var blue  = color & 255;
                var hex   = dojo.map([red, green, blue], function(c) {
    ...
    if(options.bars < 1 || options.bars > 4) {
        console.log("[Warning] Chroma-Hash expects a number parameter between 1 and 4, given " + options.bars);
    }
    
    var colors = ["primary", "secondary", "tertiary", "quaternary"].slice(0, options.bars);
    
    for(c in colors) {
        dojo.place('<label for="' + inputNode.id + '" class="' + colors[c] + ' chroma-hash"></label>', inputNode, "after");
    }
    
    var chromaHashesForElement = dojo.query("label.chroma-hash").filter(function(label) {
            return dojo.attr(label, "for") === inputNode.id;
    });
    
    chromaHashesForElement.style({backgroundColor: "#FFF", opacity: 0})

    dojo.connect(inputNode, "onkeyup", function() {
        trigger(inputNode, chromaHashesForElement)
    });
    dojo.connect(inputNode, "onblur", function() {
        trigger(inputNode, chromaHashesForElement)
    });
